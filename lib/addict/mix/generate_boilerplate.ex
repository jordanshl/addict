defmodule Mix.Tasks.Addict.Generate.Boilerplate do
  ...
  def run(_) do
    ...
    if !addict_config_already_exists?(configs_path) do
      Mix.shell().error(
        "[x] Please make sure your Addict configuration exists first. Generate it via mix:"
      )
  ...
  defp create_addict_templates do
    create_file(
      Path.join(["web", "templates", "addict", "addict.html.eex"])
      |> Path.relative_to(Mix.Project.app_path()),
      template_text()
    )

    create_file(
      Path.join(["web", "templates", "addict", "login.html.eex"])
      |> Path.relative_to(Mix.Project.app_path()),
      login_text()
    )

    create_file(
      Path.join(["web", "templates", "addict", "register.html.eex"])
      |> Path.relative_to(Mix.Project.app_path()),
      register_text()
    )

    create_file(
      Path.join(["web", "templates", "addict", "recover_password.html.eex"])
      |> Path.relative_to(Mix.Project.app_path()),
      recover_password_text()
    )

    create_file(
      Path.join(["web", "templates", "addict", "reset_password.html.eex"])
      |> Path.relative_to(Mix.Project.app_path()),
      reset_password_text()
    )
  end
  ...
  defp create_addict_view do
    view_file =
      Path.join(["web", "views", "addict_view.ex"])
      |> Path.relative_to(Mix.Project.app_path())

    create_file(
      view_file,
      view_template(base_route_helper: guess_application_name() <> ".Router.Helpers")
    )
  end
